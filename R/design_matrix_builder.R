...
design_matrix_builder <- function(anno, weight) {
  if (weight <= 0) {
    stop(paste(
      'Default weight should be positive instead of the assigned value of ',
      weight,
      '.',
      sep = ''
    ))
  ...
  if (nc_anno == 3) {
    ...
    if (sum(poscheck) > 0) {
      stop(paste(
        'Please assign positive weights for the following marker genes: \n',
        paste(
          sapply(1:nrow(anno), function(x) {
            paste(anno[x, 1], anno[x, 2], sep = ': ')
          })[poscheck],
          collapse = ', \n'
        ),
        '.',
  ...
  for (i in 1:nrow(anno)) {
    ...
    if (nc_anno == 3) {
      weightmat[
        which(marker == anno[i, 2]),
        which(celltypes == anno[i, 1])
      ] <- anno[i, 3]
    }
  }
