...
update_func <- function(
  ...
  unknown_detector <- function(dat_kn, dat_ukn) {
    ...
    calc_target2 <- function(p) {
      ...
      return(
        cost_calc2(cbind(dat_kn, dat_ukn[, pk])) +
          cost_calc2(dat_ukn[, !pk, drop = F])
      )
    }

    pl <- c(
      seq(pchisq(df + sqrt(2 * df), df), 0.9, length.out = 10),
  ...
  for (ucukn in uni_clus_ukn) {
    ...
    if (
      ...
      uknrt <- unknown_detector(
        dat_kn_add[, cluster_kn == ucukn],
        dat_ukn_add[, cluster_ukn_helper == ucukn, drop = F]
      )
    ...
    if (allclus > nc) {
      ...
      if (allclus %in% uniclus) {
        mu <- cbind(
          ...
          c(
            rep(0, lmk),
            rowMeans(dat_ukn_add[, cluster_ukn == allclus, drop = F])
          )
        )
      } else {
